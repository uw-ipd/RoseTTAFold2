...
def writepdb(f, xyz, seq, bfac=None):
    """Write protein chain to a PDB file"""
...
def TMalign(chainA, chainB):
    ...
    # run TMalign
    tm = subprocess.Popen(f'{TMALIGN} %s %s -m %s'%(fA.name, fB.name, mtx.name), 
                          shell=True, 
...
def get_tm_pairs(chains):
    """Run TM-align for all pairs of input chains"""
...
IN  = sys.argv[1] # input mmcif file (.cif or .cif.gz)
...
# save protein chains to individual .pt files
